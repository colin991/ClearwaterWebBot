...
- A player with no matching Discord member is PMed a comms reminder every minute. **Each of those PMs is followed by a jail** once they have joined a team. If they get out of jail and are still not in Discord, the next reminder PMs them again and jails them again. Players who have not picked a team yet are reminded but not jailed. A partial Discord cache is never treated as “not in Discord.”
...
The service uses `DISCORD_GUILD_ID` and `ERLC_SERVER_KEY`. Full Discord member fetches are shared and limited to **one request every 20 seconds**; VC checks, `-dc`, Sheriff balance, and other features reuse that cache. If Discord is rate-limiting, the last roster is used instead of failing `-dc`. An incomplete roster is never treated as “not in Discord” (needs about 90% of the server cached). The existing Guild Members and Guild Voice States intents are required. In-game player-list GETs refresh even while `:pm` / `:jail` commands are queued. Only in-game commands share the five-second PRC POST queue. Automatic `:load` and `:kick` are blocked unless a staff map/raw command explicitly allows them. VC checks may `:jail` when enabled.
