...
export default async function handler(request, response) {
  ...
  try {
    if (request.method === 'GET') {
      ...
      const result = await callBot(request, undefined, viewer.id, hashClientIp(request));
      const feedBody = redactPublicPayload(result.body);
      if (result.ok) setCached(feedKey, feedBody, FEED_CACHE_TTL_MS);
      return sendJson(response, result.ok ? 200 : result.status, feedBody);
    }
