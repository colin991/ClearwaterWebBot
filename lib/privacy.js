...
export function encryptString(plain, secret = piiSecret()) {
...
export function proxiedMediaUrl(value, secret = piiSecret()) {
  ...
  return `/api/media?t=${encodeURIComponent(encryptString(value, secret))}`;
}
...
function redactObject(value, secret) {
  if (value == null) return value;
  if (Array.isArray(value)) {
    return value.map((entry) => (isSnowflake(entry) ? publicUserId(entry, secret) : redactObject(entry, secret)));
  }
  ...
  for (const [key, entry] of Object.entries(value)) {
    if (key === 'ipHashes' || key === 'ipHash' || key === 'ipHashLegacy') continue;
    if (key === 'votes' && entry && typeof entry === 'object' && !Array.isArray(entry)) {
      out[key] = Object.fromEntries(
        Object.entries(entry).map(([id, vote]) => [isSnowflake(id) ? publicUserId(id, secret) : id, vote]),
      );
      continue;
    }
    if (typeof entry === 'string' && ID_KEYS.has(key) && isSnowflake(entry)) {
    ...
    out[key] = redactObject(entry, secret);
  }
  return out;
}
...
export function redactPublicPayload(body, extra = {}) {
  const secret = piiSecret();
  const redacted = redactObject(body, secret) || {};
  return {
    ...redacted,
...
export async function resolvePublicIds(body, users = []) {
  ...
  const next = { ...body };
  for (const key of ['withUserId', 'targetId', 'to']) {
