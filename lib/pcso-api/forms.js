...
export default async function handler(request, response) {
  ...
  try {
    ...
    if (!posted.ok && posted.reason !== 'not_configured') {
    ...
    const status = /Sign in/.test(String(error?.message || '')) ? 401 : 400;
    return sendJson(response, status, { error: error?.message || 'The form could not be submitted.' });
  }
}
