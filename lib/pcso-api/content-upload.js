...
const CONTENT_PATH_RE = /^pcso-content\/(news|events|star)\/[a-z0-9._-]+$/i;
    ...
    try {
      const result = await handleUpload({
        ...
        onBeforeGenerateToken: async (pathname) => {
          ...
          const isStar = path.startsWith('pcso-content/star/');
          return {
            allowedContentTypes: isStar
              ? ['image/png', 'image/jpeg', 'image/webp', 'image/gif', 'video/mp4', 'video/webm', 'video/quicktime']
              : ['image/png', 'image/jpeg', 'image/webp', 'image/gif'],
            maximumSizeInBytes: isStar ? MAX_MEDIA_BYTES : MAX_IMAGE_BYTES,
