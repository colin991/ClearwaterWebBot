...
export default async function handler(request, response) {
  try {
    ...
    if (request.method === 'DELETE') {
      const kind = body.kind === 'event' ? 'event' : (body.kind === 'star' ? 'star' : 'news');
    ...
    if (body.kind === 'star') {
      if (!String(body.title || '').trim() && !String(body.body || '').trim() && !String(body.mediaUrl || '').trim()) {
        return sendJson(response, 400, { error: 'Add text, an image, or a video.' });
      }
      const content = await addPcsoStarItem(body);
      return sendJson(response, 200, { ok: true, ...content });
    }
