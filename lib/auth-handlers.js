...
import { checkClientVpn, rejectVpnRedirect } from './vpn-guard.js';

export async function handleDiscordStart(request, response) {
  if (request.method !== 'GET') return sendJson(response, 405, { error: 'Method not allowed' });

  try {
    const requestUrl = new URL(request.url, `https://${request.headers.host || 'cwrpvc.lol'}`);
    const next = safeNextPath(requestUrl.searchParams.get('next') || '');
    // Do not VPN-block here — identity is unknown until Discord returns.
    // Allowlisted users need to complete OAuth while on a VPN.
...
export async function handleDiscordCallback(request, response) {
  ...
  try {
    ...
    // VPN check after Discord identity is known so allowlisted users can sign in on a VPN.
    if (await rejectVpnRedirect(request, response, '/internet', user.id)) {
      return;
    }
...
export async function handleMe(request, response) {
  ...
  try {
    ...
    const vpn = await checkClientVpn(request, user?.id);
    if (vpn.blocked) {
      return sendJson(response, 200, {
        authenticated: false,
        siteAccess: false,
        vpnBlocked: true,
        error: 'VPNs and proxies are not allowed on Clearwater.',
      });
    }
    ...
    return sendJson(response, 200, {
      ...
      user: {
        ...
        governmentAccess: staffAccess.governmentAccess === true,
        governmentReview: staffAccess.governmentReview === true,
