...
export async function handleDiscordStart(request, response) {
  ...
  try {
    ...
    if (await rejectVpnRedirect(request, response, next)) return;
...
export async function handleDiscordCallback(request, response) {
  ...
  try {
    if (await rejectVpnRedirect(request, response, '/internet')) return;
...
export async function handleMe(request, response) {
  ...
  try {
    const vpn = await checkClientVpn(request);
    if (vpn.blocked) {
