...
export async function getStaffAccess(user) {
  ...
  // Developer badge = every website permission (owner panel, staff, management, government).
  if (hasFullWebsiteAccess(user)) return developerFullAccess(user);
  ...
  try {
    ...
    // Bot may report the developer badge from Discord roles / internet store.
    if (hasFullWebsiteAccess({ ...user, badges: result.badges })) {
