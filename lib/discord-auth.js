...
export const SAFE_NEXT_PATHS = Object.freeze([
  ...
  '/employee/department',
  '/employee-department.html',
  '/employee/reports',
  '/employee-reports.html',
  '/employee/command',
  '/employee-command.html',
]);
...
/**
 * Build the OAuth redirect URI for the current request host.
 * Discord requires an exact match in the application’s Redirects list.
 */
export function resolveRedirectUri(request, configuredUri = process.env.DISCORD_REDIRECT_URI) {
  ...
  const derived = host ? `${proto}://${host}/api/auth/discord/callback` : '';

  // Prefer the live request host so redirect_uri matches the domain being used.
  // That exact URL must also be listed under Discord Developer Portal → OAuth2 → Redirects.
  if (derived) return derived;
  return String(configuredUri || '').trim();
}

export function getAuthConfig(request = null) {
  const config = {
    clientId: process.env.DISCORD_CLIENT_ID,
    clientSecret: process.env.DISCORD_CLIENT_SECRET,
    sessionSecret: process.env.SESSION_SECRET,
    redirectUri: resolveRedirectUri(request, process.env.DISCORD_REDIRECT_URI),
  };

  const missing = Object.entries(config)
    .filter(([key, value]) => key === 'redirectUri' ? !value : !value)
    .map(([key]) => key);
...
function sessionPayload(user) {
  return {
    ...
    pinellasRoles: Array.isArray(user.pinellasRoles)
      ? user.pinellasRoles.filter((roleId) => /^\d{16,22}$/.test(String(roleId))).map(String).slice(0, 100)
      : [],
    discordAdmin: user.discordAdmin === true,
    pinellasMember: user.pinellasMember === true,
...
export function sessionIsWebsiteSignedIn(user, staffAccess = null) {
  if (!user?.id) return false;
  if (staffAccess?.siteAccess) return true;
  if (user.pinellasMember === true) return true;
  return false;
}
