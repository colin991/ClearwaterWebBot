...
export function safeNextPath(value = '') {
  if (!SAFE_NEXT_PATHS.includes(value)) return '/';
  if (value === '/internet.html') return '/internet';
  if (value === '/owner.html') return '/owner';
  return value;
}
...
function sessionPayload(user) {
  return {
    ...
    // Short-lived sessions force Discord re-login so removed tester/staff roles
    // cannot keep working for a full week off a stale cookie.
