...
/** Discord user IDs that may use Clearwater on a VPN/proxy. */
const VPN_ALLOW_USER_IDS = new Set([
  '1074411240757137589',
  ...String(process.env.VPN_ALLOW_USER_IDS || '')
    .split(/[\s,]+/)
    .map((id) => id.trim())
    .filter(Boolean),
]);

export function isVpnAllowlistedUser(userId) {
  const id = String(userId || '').trim();
  return Boolean(id) && VPN_ALLOW_USER_IDS.has(id);
}
 ...
 * Pass userId when known so allowlisted members can use a VPN.
 */
export async function checkClientVpn(request, userId = '') {
  if (isVpnAllowlistedUser(userId)) {
    return { blocked: false, skipped: true, reason: 'allowlisted_user', userId: String(userId) };
  }
...
export async function rejectVpnJson(request, response, userId = '') {
  const verdict = await checkClientVpn(request, userId);
...
export async function rejectVpnRedirect(request, response, nextPath = '/', userId = '') {
  const verdict = await checkClientVpn(request, userId);
  if (!verdict.blocked) return false;
