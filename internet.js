...
let messageGif = null;
// A Reel should try to play with sound as it becomes the focused Reel. Browsers
// can still require a first gesture before they allow audible autoplay; in that
// case we fall back to a muted video without changing the user's sound choice.
let reelsSoundOn = true;
...
function setReelSound(on) {
  ...
  document.querySelectorAll('[data-reels-viewport] video').forEach((video) => {
    const isActive = video.closest('.reel-card')?.dataset.reelId === activeReelMediaId;
    video.muted = !reelsSoundOn || !isActive;
  });
...
function toggleReelSound(card) {
  ...
  activeReelMediaId = card.dataset.reelId || null;
  video.muted = false;
  void video.play().catch(() => {
    // Browsers can initially block audible autoplay. Keep the person's sound
    // preference and continue this Reel silently until the next interaction.
    video.muted = true;
    void video.play().catch(() => {});
  });
}
...
function playReelVideo(video) {
  ...
  void video.play().then(() => {
    ...
    // Keep the user's sound choice. A browser may require one click before
    // it allows sound, but the centered Reel should still start playing.
    video.muted = true;
    void video.play().catch(() => {});
  });
}
...
function renderReels() {
  ...
  viewport.innerHTML = reels.map((reel, reelIndex) => {
    const likes = Array.isArray(reel.likes) ? reel.likes : [];
    const liked = likes.includes(activeUserId());
    const comments = reelCommentsFor(reel.id).length;
    const author = internetUsers.get(reel.authorId) || {};
    ...
    const media = videoSrc
      ? `<video src="${escapeHtml(videoSrc)}" loop muted playsinline webkit-playsinline preload="${reelIndex === 0 ? 'auto' : 'metadata'}" autoplay></video>`
...
async function submitReelComment(input) {
  ...
  try {
    ...
    await postInteraction({ postId: reelId, type: 'reply', content: text }, { reload: false });
    // Add the new comment locally so the current video element is never
    // recreated just to show a comment. The normal background refresh will
    // reconcile it with the bot-host store shortly afterwards.
    const author = internetUsers.get(activeUserId()) || {};
    allPosts.push({
    ...
    const reel = allPosts.find((post) => post.id === reelId);
    const card = document.querySelector(`.reel-card[data-reel-id="${CSS.escape(reelId)}"]`);
    if (reel) updateReelStats(card, reel);
    renderReelPanel(reelId);
    const sheet = document.querySelector('[data-reel-comments]');
    if (sheet && !sheet.hidden) openReelComments(reelId);
  } finally {
