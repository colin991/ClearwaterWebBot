...
async function sendWarning(channel, discordId, minutes, name) {
  ...
  const content = minutes === 5
    ? `⚠️ <@${discordId}>${label} has been stationary in ER:LC for more than **5 minutes** while on shift.`
    : `🚨 <@${discordId}>${label} has been stationary in ER:LC for more than **10 minutes** while on shift.`;
