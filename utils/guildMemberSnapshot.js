...
export function createMemberSnapshotLoader({ now = Date.now, ttlMs = 300000 } = {}) {
  ...
  return async function ensureGuildMembers(guild) {
    ...
    if (now() < state.retryAt) throw new Error(`Discord member list is cooling down. Please try again in ${Math.ceil((state.retryAt - now()) / 1000)} seconds.`);
