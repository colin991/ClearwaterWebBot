...
export function publicUsers(store, viewerId) {
  ...
  const people = users.map((user) => {
    ...
    return {
      ...
      hideLikes: user.preferences?.hideLikes === true,
...
export function addInternetFollowers(store, { actorId, handle, discordId, accountId, amount, actor } = {}) {
  ...
  if (/^\d{16,22}$/.test(pingId)) {
    ...
    const matches = Object.values(store.users).filter((member) => String(member.username || '').toLowerCase() === name);
    if (matches.length !== 1) {
      throw new Error(matches.length ? 'That handle matches more than one account.' : 'Account not found.');
    }
    user = matches[0];
  }
